...
export async function obtenerUsuarioPorCorreo(correo) {
  ...
  const [filas] = await pool.execute(sql, [correo]);

  return filas[0];
}

export async function obtenerUsuarioPorNombreUsuario(nombreUsuario) {
  const sql = `
    SELECT id
    FROM usuarios
...
export async function obtenerDepartamentoPorId(departamentoId) {
  const sql = `
    SELECT id
    FROM departamentos
    WHERE id = ?
  ...
  const [filas] = await pool.execute(sql, [departamentoId]);
...
export async function actualizarPerfil(usuarioId, datosUsuario) {
  const sql = `
    UPDATE usuarios
    SET
      nombre_completo = ?,
...
export async function actualizarPasswordHash(usuarioId, passwordHash) {
  const sql = `
    UPDATE usuarios
    SET password_hash = ?
    WHERE id = ?
  ...
  const [resultado] = await pool.execute(sql, [passwordHash, usuarioId]);
